...
```
--
.hex-sticker[![](../slides/images/pipe.png)]
---
class: inverse
...
Validity can be further sub-categorized, and we will spend time checking each of these aspects of your data using R and the Tidyverse.

  * Data-Type Constraints
  * <mark>Range Constraints</mark>
...
Validity can be further sub-categorized, and we will spend time checking each of these aspects of your data using R and the Tidyverse.
  ...
  * Data-Type Constraints
  * Range Constraints
  * <mark>Mandatory Constraints</mark> 
  * Unique Constraints  
  * Set-Membership  
  * Regular Expressions  
  * Cross-field validation 
...
Missing data will be dealt with more fully later.

```{r eval = FALSE}
n <- 10
data_mand_constr <- tibble(id = 1:n,
                           time_1 = rnorm(n),
                           time_2 = rnorm(n),
                           time_3 = rnorm(n)) %>% 
...
For instance, each subject should have their own ID such that all data are properly attributed to that subject. Same goes for structurally nested data like education data where students are studied within classrooms, with schools.
...
Let's try checking this out using the superhero data set (exercises_section_1.R)

```{r}
# check how many rows in the data
# This is assuming that each row represents a unique subject
nrow(data_superhero)

# How many unique values are in the uniquely constrained variable?
n_distinct(data_superhero$id)
```
---
class:inverse
...
This approach can also be used to check for uniqueness across multiple variables.
...
# How many unique values are in the uniquely constrained variable?
n_distinct(data_superhero$name)
```

???

Keep in mind that as long as one of the variables checked this way is completely unique, then the result will indicate that the set of variables will also be unique.

Check individual variables first.
---
class: inverse

We can also build chains of functions to look deeper into the `name` issue we just found.

```{r echo=FALSE}
# How many uniques values are there in a vector?
# This is equivalent to length(unique(data_superhero$ID))
n_distinct(data_superhero$id)

unique_names <- data_superhero %>% 
  distinct(name)

# Alternatively, we can tabulate the number of rows with unique ID using the count() function
# We can then chain that by filtering for any value in the tbale with n > 1, that is more than 1 row having the same ID value (it should return a tibble with zero rows)
dupe_names <- data_superhero %>% 
  count(name) %>% 
  filter(n > 1) %>% 
  pull(name)
...
# Maybe we want to find rows or dupe_names which only contain a name
data_superhero %>% 
  naniar::add_n_miss()
```
---
...
We could also assess this visually.
``` {r echo = FALSE}
# take our tabular data (as a tibble), and plot it
p <- data_superhero %>% 
  ggplot(aes(x = id)) +
  geom_bar()
...
This refers to categorical/discrete data, or FACTORS. Factors should be used when there is a known and fixed set of values which can be observed. Sometimes, you might have a sense of the possible values, but there could be more. Consider an open ended questions about gender identity or more trivially a subject's favourite colour. I might be hard to include options for the numerous way a subject might respond prior to seeing your data.

--

The Forcats package has made my R life much easier.

We briefly touched upon the readr::parse_factor() function already, but there are many other operations we might want to do with categorical data. Especially, if it is hard to foresee all of the possible levels in a variable.

???

Forcats is an anagram of the word factors.
---
class: inverse
...
If we want to get a tabular count of observations per grouping level, but if you have a many possible levels in your factor, then you might want to consolidate (for diagnostic or analytic purposes).

Keep in mind that the resulting levels should still have substantive meaning (if you can help it).

Take for instance, that for eye colours for superheroes are predominantly blue, green, or brown.
```{r}
fct_count(data_superhero$eye_color) %>% 
  arrange(desc(n))

# We can say that any color with only a few (3 or less count could be group into an "other" catagory)
fct_lump_min(data_superhero$eye_color,min = 3) %>% 
  fct_infreq() %>% 
  tibble %>% 
  ggplot(aes(x = 1)) +
  geom_bar()
...
.pull-left[
```{r}
n_distinct(data_superhero$eye_color)
```
]
...
``` {r}
data_superhero %>% 
  ggplot(aes(x = eye_color, group = alignment)) +
  geom_bar() +
